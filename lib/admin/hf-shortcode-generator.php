...
?>
	<div class="hf-shortcode-container hf-shortcode-select">
		<div class="hf-shortcode-title">Choose shortcode</div>
		...
		<form name="general_form" method="post" action="admin.php?page=hellofollowers&tab=shortcode">
		<input type="hidden" id="cmd" name="cmd" value="generate" />
		<input type="hidden" id="code" name="code" value="<?php echo $active_shortcode; ?>"/>
 			<?php wp_nonce_field('hellofollowers_shortcode'); ?>
			<div class="hf-options">
			<div class="hf-options-header" id="hf-options-header">
				<div class="hf-options-title">
					Shortcode Generator
				</div>		
				<input type="Submit" name="Submit" value="<?php esc_html_e( 'Generate Shortcode', 'hellofollowers' ); ?>" class="button-primary" />
			</div>
