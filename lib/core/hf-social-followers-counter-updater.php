...
class HFSocialFollowersCounterUpdater {
	...
	public function __construct() {
		if (! class_exists ( 'OAuthServer' )) {
			require_once HFOLLOW_PLUGIN_ROOT . 'lib/core/OAuth/OAuth.php';
		}
	}
	...
	public function update_twitter() {
		...
		if (! class_exists ( 'TwitterOAuth' )) {
			include_once HFOLLOW_PLUGIN_ROOT . 'lib/core/twitter/twitteroauth.php';
		}
	...
	private function update_facebook_page() {
		try {
			return $this->update_facebook_without_token(HFSocialFollowersCounterHelper::get_option ( 'facebook_id' ));
		} catch ( Exception $e ) {
			return 0;
		}
	...
	private function update_facebook_without_token($id) {
		...
		$get_request = wp_remote_get( "https://www.facebook.com/plugins/likebox.php?href=https://facebook.com/$id&show_faces=true&header=false&stream=false&show_border=false&locale=en_US", array( 'timeout' => 20 ) );
	...
	public function update_pinterest() {
		...
		try {
			$request = @$this->remote_update ( 'https://api.pinterest.com/v3/pidgets/users/'.$id.'/pins/' );
	...
	public function update_linkedin() {
		...
		$account_type = HFSocialFollowersCounterHelper::get_option ( 'linkedin_account_type' );
		$api_key = HFSocialFollowersCounterHelper::get_option ( 'linkedin_api_key' );
		...
		if ($account_type != 'user') {
			try {
				$args = array('headers' => array('Authorization' => sprintf('Bearer %s', $api_key)));
					
				$response = wp_remote_get('https://api.linkedin.com/v1/companies/'.$id.'/num-followers?format=json', $args);
				
				if (!is_wp_error($response)) {							
					$response_result = json_decode(wp_remote_retrieve_body($response), true);
				
					if (!$response_result || is_array($response_result)) {			
						$result = 0;
					}
					else {
			...
			// since @1.1
			// getting data for personal profiles
			$args = array('headers' => array('Authorization' => sprintf('Bearer %s', $api_key)));
			...
			if (!is_wp_error($response)) {							
				...
				if (!$response_result || !isset($response_result['numConnections'])) {			
					$result = 0;
				}
				else {
					$result = $response_result['numConnections'];
				}
			}	
		}
	...
	public function update_mailchimp() {
		...
		if (! class_exists ( 'MCAPI' )) {
			require_once HFOLLOW_PLUGIN_ROOT . 'lib/core/mailchimp/MCAPI.class.php';
		}
	...
	public function update_vine() {
		...
		if (! class_exists ( 'VineApp' )) {
			require_once HFOLLOW_PLUGIN_ROOT . 'lib/core/vine/Vine.php';
		}
	...
	public function update_tumblr() {
		...
		if (! class_exists ( 'Tumblr' )) {
			require_once HFOLLOW_PLUGIN_ROOT . 'lib/core/Tumblr/Tumblr.php';
		}
