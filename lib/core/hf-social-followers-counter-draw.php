...
class HFSocialFollowersCounterDraw {
	...
	public static function draw_followers($options, $draw_title = false) {
		...
		if (!empty($instace_customizer)) {
			$instace_customizer = ' hf-customizer-'.esc_attr($instace_customizer);
		}
		
		if (!empty($instance_cover_bgcolor)) {
			$instance_cover_bgcolor = 'background-color:'.esc_attr($instance_cover_bgcolor).';';
		}
		if (!empty($instance_cover_bgimage)) {
			$instance_cover_bgimage = 'background-image: url('.esc_url($instance_cover_bgimage).');';
		}
		...
		$class_template = (! empty ( $instance_template )) ? " hf-template-" . esc_attr($instance_template) : '';
		$class_animation = (! empty ( $instance_animation )) ? " hf-icon-" . esc_attr($instance_animation) : '';
		$class_columns = (! empty ( $instance_columns )) ? " hf-col-" . esc_attr($instance_columns) : '';
		...
		if ($draw_title && ! empty ( $instance_title )) {
			printf ( '<h3>%1$s</h3>', esc_html($instance_title) );
		}
		...
		$active_social_networks = hellofollowers_followers()->active_social_networks();
		$user_layout_active = false;
		$user_layout_total = 0;
		if (strpos($instance_columns, 'user') !== false) {
			$layout_id = str_replace('user', '', $instance_columns);
			$active_social_networks = hellofollowers_followers()->active_layout_social_networks($layout_id);
		...
		if (intval($instance_cover) == 1) {
			echo '<div class="hf-cover hf-cover-style-'.esc_attr($instance_cover_style).'" style="'.$instance_cover_bgcolor.$instance_cover_bgimage.'">';
		...
		foreach ( $active_social_networks as $social ) {
			...
			$follow_url = hellofollowers_followers()->create_follow_address ( $social );
			if (! empty ( $follow_url )) {
			...
			printf ( '<span class="hf-followers-text">%1$s</span>', esc_html($social_followers_text) );
			echo '</div>';
			
			if (! empty ( esc_url($follow_url) )) {
				echo '</a>';
			}
			echo '</li>';
		}
	...
	public static function cover_title($title, $fans_total = '') {
		...
		if (!empty($title)) {
			...
			return '<div class="hf-cover-title">'.esc_html($title).'</div>';
		}
		else {
			return "";
		}
	...
	public static function cover_text($title, $fans_total = '') {
		...
		if (!empty($title)) {
			$title = str_replace('{TOTAL}', $fans_total, $title);
			return '<div class="hf-cover-text">'.esc_html($title).'</div>';
		}
		else {
			return "";
		}
	...
	public static function cover_image($value, $style) {
		...
		if (!empty($value)) {
			return '<div class="hf-cover-image'.esc_attr($style).'"><img class="" src="'.esc_url($value).'" /></div>';
		}
		else {
			return "";
		}
