...
class HFSocialFollowersCounter {
	...
	private $version = "1.0";
	private $hellofollowers_cache_option_name = "hffcounter_cached";
	private $hellofollowers_expire_name = "hffcounter_expire";
	private $updater_instance;
	
	function __construct() {

		// include updater class
		include_once (HFOLLOW_PLUGIN_ROOT . 'lib/core/hf-social-followers-counter-updater.php');
		
		// include visual draw class
		include_once (HFOLLOW_PLUGIN_ROOT . 'lib/core/hf-social-followers-counter-draw.php');
		...
		add_shortcode( 'hello-follower-count' , array ( $this , 'register_plugin_shortcode_count' ) );
	...
	public function register_front_assets() {
		wp_register_style ( 'hello-followers', HFOLLOW_PLUGIN_URL . '/assets/css/hello-followers.min.css', array (), HFOLLOW_VERSION );
		wp_enqueue_style ( 'hello-followers' );
		
		wp_register_style ( 'hello-followers-animation', HFOLLOW_PLUGIN_URL . '/assets/css/hover.min.css', array (), HFOLLOW_VERSION );
	...
	public function generate_customizer() {
		...
		// single color
		$hellofollowers_monotone_bgcolor1 = HFSocialFollowersCounterHelper::get_option('monotone_bgcolor1');
		$hellofollowers_monotone_bgcolor2 = HFSocialFollowersCounterHelper::get_option('monotone_bgcolor2');
		$hellofollowers_monotone_color1 = HFSocialFollowersCounterHelper::get_option('monotone_color1');
		$hellofollowers_monotone_color2 = HFSocialFollowersCounterHelper::get_option('monotone_color2');
		
		$all_networks = HFSocialFollowersCounterHelper::available_social_networks();

			if ($hellofollowers_monotone_bgcolor1 != '') {
				$output .= '.hf-customizer-mono li[class^="hf-"] .hf-network { background-color: '.esc_attr($hellofollowers_monotone_bgcolor1).'!important;}';
			}
			if ($hellofollowers_monotone_bgcolor2 != '') {
				$output .= '.hf-customizer-mono li[class^="hf-"] .hf-network:hover { background-color: '.esc_attr($hellofollowers_monotone_bgcolor2).'!important;}';
			}
			if ($hellofollowers_monotone_color1 != '') {
				$output .= '.hf-customizer-mono li[class^="hf-"] .hf-network { color: '.esc_attr($hellofollowers_monotone_color1).'!important;}';
			}
			if ($hellofollowers_monotone_color2 != '') {
				$output .= '.hf-customizer-mono li[class^="hf-"] .hf-network:hover { color: '.esc_attr($hellofollowers_monotone_color2).'!important;}';
			}
				
		
		foreach ($all_networks as $network => $title) {
			// multi color
			$hellofollowers_network_bgcolor1 = HFSocialFollowersCounterHelper::get_option($network.'_bgcolor1');
			$hellofollowers_network_bgcolor2 = HFSocialFollowersCounterHelper::get_option($network.'_bgcolor2');
			$hellofollowers_network_color1 = HFSocialFollowersCounterHelper::get_option($network.'_color1');
			$hellofollowers_network_color2 = HFSocialFollowersCounterHelper::get_option($network.'_color2');

			if ($hellofollowers_network_bgcolor1 != '') {
				$output .= '.hf-customizer-color .hf-'.$network.' .hf-network { background-color: '.esc_attr($hellofollowers_network_bgcolor1).'!important;}';
			}
			if ($hellofollowers_network_bgcolor2 != '') {
				$output .= '.hf-customizer-color .hf-'.$network.' .hf-network:hover { background-color: '.esc_attr($hellofollowers_network_bgcolor2).'!important;}';
			}
			if ($hellofollowers_network_color1 != '') {
				$output .= '.hf-customizer-color .hf-'.$network.' .hf-network { color: '.esc_attr($hellofollowers_network_color1).'!important;}';
			}
			if ($hellofollowers_network_color2 != '') {
				$output .= '.hf-customizer-color .hf-'.$network.' .hf-network:hover { color: '.esc_attr($hellofollowers_network_color2).'!important;}';
			}
		...
		for ($layout_id=1;$layout_id <= 5;$layout_id++) {
			...
			$output .= '.hf-container.hf-col-user'.esc_attr($layout_id).' li { width: '.esc_attr($width).'% !important; display: inline-block; }';
			foreach ($all_networks as $network => $title) {
				...
				if ($network_width != '' && $network_width != '1') {
					...
					$output .= '.hf-container.hf-col-user'.esc_attr($layout_id).' li.hf-'.esc_attr($network).' { width: '.esc_attr($network_percentage).'% !important; display: inline-block; }';
		...
		if ($output != '') {
			wp_add_inline_style('hello-followers', $output);
		}
	}
	...
	public function register_plugin_shortcode_count($atts = array()) {
		$social = isset($atts['social']) ? $atts['social'] : '';
		$short = isset($atts['short']) ? $atts['short'] : '';
		$social_followers_counter = '';
		
		if ($social != '') {
			$followers_count = hellofollowers_followers ()->get_followers ();
			$social_followers_counter = isset ( $followers_count [$social] ) ? $followers_count [$social] : 0;
			
			if ($short == 'yes' || $short == 'true') {
				$social_followers_counter = HFSocialFollowersCounterDraw::followers_number_shorten($social_followers_counter);
			}
		}
		
		return $social_followers_counter;
	}
	...
	public function require_counter_update() {
		$expire_time = get_option ( $this->hellofollowers_expire_name );
		$now = time ();
	...
	public function get_followers() {
		...
		if (!$request_update) {
			$counters = get_option ( $this->hellofollowers_cache_option_name );
	...
	public function clear_stored_values() {
		delete_option($this->hellofollowers_cache_option_name);
	}
	
	
	public function settle_immediate_update() {
		delete_option($this->hellofollowers_expire_name);
	}
	...
	public static function create_follow_address($social) {
		...
		switch ($social) {
			case 'facebook' :
				return 'https://www.facebook.com/' . esc_attr(HFSocialFollowersCounterHelper::get_option ( $social . '_id' ));
				break;
			case 'twitter' :
				return 'https://www.twitter.com/' . esc_attr(HFSocialFollowersCounterHelper::get_option ( $social . '_id' ));
				break;
			case 'google' :
				return 'https://plus.google.com/' . esc_attr(HFSocialFollowersCounterHelper::get_option ( $social . '_id' ));
				break;
			case 'pinterest' :
				return 'https://www.pinterest.com/' . esc_attr(HFSocialFollowersCounterHelper::get_option ( $social . '_id' ));
				break;
			case 'linkedin' :
				return esc_url(HFSocialFollowersCounterHelper::get_option ( $social . '_id' ));
				break;
			case 'github' :
				return 'http://github.com/' . esc_attr(HFSocialFollowersCounterHelper::get_option ( $social . '_id' ));
				break;
			case 'vimeo' :
				if (HFSocialFollowersCounterHelper::get_option ( $social . '_account_type', 'channel' ) == 'user') {
					{
						...
						if (preg_match ( '/^[0-9]+$/', $vimeo_id )) {
							return 'http://vimeo.com/user' . esc_attr($vimeo_id);
						} else {
							return 'http://vimeo.com/' . esc_attr($vimeo_id);
						}
					}
				} else {
					return 'http://vimeo.com/channels/' . esc_attr(HFSocialFollowersCounterHelper::get_option ( $social . '_id' ));
				}
				break;
			case 'dribbble' :
				return 'http://dribbble.com/' . esc_attr(HFSocialFollowersCounterHelper::get_option ( $social . '_id' ));
				break;
			case 'soundcloud' :
				return 'https://soundcloud.com/' . esc_attr(HFSocialFollowersCounterHelper::get_option ( $social . '_id' ));
				break;
			case 'behance' :
				return 'http://www.behance.net/' . esc_attr(HFSocialFollowersCounterHelper::get_option ( $social . '_id' ));
				break;
			case 'foursquare' :
				if (intval ( HFSocialFollowersCounterHelper::get_option ( $social . '_id' ) ) && intval ( HFSocialFollowersCounterHelper::get_option ( $social . '_id' ) ) == HFSocialFollowersCounterHelper::get_option ( $social . '_id' )) {
					return 'https://foursquare.com/user/' . esc_attr(HFSocialFollowersCounterHelper::get_option ( $social . '_id' ));
				} else {
					return 'https://foursquare.com/' . esc_attr(HFSocialFollowersCounterHelper::get_option ( $social . '_id' ));
				}
				break;
			case 'forrst' :
				return 'http://forrst.com/people/' . esc_attr(HFSocialFollowersCounterHelper::get_option ( $social . '_id' ));
				break;
			case 'mailchimp' :
				return esc_url(HFSocialFollowersCounterHelper::get_option ( $social . '_list_url' ));
				break;
			case 'delicious' :
				return 'https://delicious.com/' . esc_attr(HFSocialFollowersCounterHelper::get_option ( $social . '_id' ));
				break;
			case 'instgram' :
			case 'instagram' :
				return 'http://instagram.com/' . esc_attr(HFSocialFollowersCounterHelper::get_option ( $social . '_username' ));
				break;
			case 'youtube' :
				return 'https://www.youtube.com/' . esc_attr(HFSocialFollowersCounterHelper::get_option ( $social . '_account_type' )) . '/' . esc_attr(HFSocialFollowersCounterHelper::get_option ( $social . '_id' ));
				break;
			case 'envato' :
				$ref = '';
				if (HFSocialFollowersCounterHelper::get_option ( $social . '_ref' )) {
					$ref = '?ref=' . esc_attr(HFSocialFollowersCounterHelper::get_option ( $social . '_ref' ));
				}
				return 'http://www.' . HFSocialFollowersCounterHelper::get_option ( $social . '_site' ) . '.net/user/' . esc_attr(HFSocialFollowersCounterHelper::get_option ( $social . '_id' )) . $ref;
				break;
			case 'vk' :
				$account_type = HFSocialFollowersCounterHelper::get_option ( $social . '_account_type' );
				if ($account_type == "community") {
					return 'http://www.vk.com/' . esc_attr(HFSocialFollowersCounterHelper::get_option ( $social . '_id' ));
				}
				else {
					return 'http://www.vk.com/id' . esc_attr(HFSocialFollowersCounterHelper::get_option ( $social . '_id' ));
				}
				break;
			case 'rss' :
				return esc_url(HFSocialFollowersCounterHelper::get_option ( $social . '_link' ));
				break;
			case 'vine' :
				return 'https://vine.co/' . esc_attr(HFSocialFollowersCounterHelper::get_option ( $social . '_username' ));
				break;
			case 'tumblr' :
				...
				if ($basename2arr == 'www')
					...
					return 'http://www.tumblr.com/follow/' . esc_attr(@$basename2arr [0]);
				break;
			case 'slideshare' :
				return 'http://www.slideshare.net/' . esc_attr(HFSocialFollowersCounterHelper::get_option ( $social . '_username' ));
				break;
			case '500px' :
				return 'http://500px.com/' . esc_attr(HFSocialFollowersCounterHelper::get_option ( $social . '_username' ));
				break;
			case 'flickr' :
				return 'https://www.flickr.com/photos/' . esc_attr(HFSocialFollowersCounterHelper::get_option ( $social . '_id' ));
				break;
			case 'wp_posts' :
			case 'wp_users' :
			case 'wp_comments' :
				return esc_url(HFSocialFollowersCounterHelper::get_option ( $social . '_url' ));				
				break;
			case 'audioboo' :
				return 'https://audioboo.fm/users/' . esc_attr(HFSocialFollowersCounterHelper::get_option ( $social . '_id' ));
				break;
			case 'steamcommunity' :
				return 'http://steamcommunity.com/groups/' . esc_attr(HFSocialFollowersCounterHelper::get_option ( $social . '_id' ));
				break;
			case 'weheartit' :
				return 'http://weheartit.com/' . esc_attr(HFSocialFollowersCounterHelper::get_option ( $social . '_id' ));
				break;
			case 'love' :
				return esc_url(HFSocialFollowersCounterHelper::get_option ( $social . '_url' ));
				break;
			...
			case 'mailpoet':
				...
				break;
			case 'twitch' :
				return 'http://www.twitch.tv/' . esc_attr(HFSocialFollowersCounterHelper::get_option ( $social . '_id' )).'/profile';
				break;
			case 'spotify' :
				return esc_url(HFSocialFollowersCounterHelper::get_option ( $social . '_id' ));
				break;
