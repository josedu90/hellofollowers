...
class HelloFollowersOptionsFramework {
	...
	public static function draw_options_field($option, $custom = false, $user_settings = array()) {		
		$hellofollowers_options = hellofollowers_options();
		...
		$settings_group = "hellofollowers_options";
		$is_fans_counter = false;
		$option_value = isset($hellofollowers_options[$id]) ? $hellofollowers_options[$id] : '';
		...
		switch ($type) {
			...
			case "checkbox_list_sortable":
				$ordered_values = isset($hellofollowers_options[$id.'_order']) ? $hellofollowers_options[$id.'_order'] : array();
	...
	public static function draw_simplesort_field($field, $listOfValues, $settings_group = 'hellofollowers_options', $value = '') {
		if ($value == '') {
			$value = $listOfValues; // initialize default values
		}
		
		printf('<ul class="hellofollowers_sortable" id="hf-sortable-%1$s">', $field);
		
		foreach ($value as $single) {
			printf('<li><i class="fa fa-bars"></i> %1$s<input type="hidden" name="%2$s[%3$s][]" value="%1$s"/></li>', $single, $settings_group, $field);
		}
		
		echo '</ul>';
	}
	
	public static function draw_image_checkbox_field($field, $listOfValues, $settings_group = 'hellofollowers_options', $value = '') {
		...
		echo '<div class="hellofollowers_image_checkbox_container hellofollowers_image_checkbox_container_'.$field.'">';
		foreach ( $listOfValues as $singleValueCode => $singleValue ) {
			...
			if ($label != '') {
				$label = sprintf('<div class="hellofollowers_checkbox_label">%1$s</div>', $label);
			}	
			
			printf('<div class="hellofollowers_checkbox"><div class="hellofollowers_image_checkbox%6$s" data-field="%8$s_%3$s">
					<span class="checkbox-image"><img src="%1$s/%2$s"/></span>
					<span class="checkbox-state"><i class="fa fa-lg fa-check-circle"></i></span>
					<input type="checkbox" id="hellofollowers_options_%8$s_%3$s" name="%4$s[%8$s][]" value="%3$s"%7$s/>					
					</div>%5$s</div>', ESSB3_PLUGIN_URL, $singleValue['image'], $singleValueCode, $settings_group, $label, $active_state, $active_element, $field);
		}	
		echo '</div>';
	}

	public static function draw_image_radio_field($field, $listOfValues, $settings_group = 'hellofollowers_options', $value = '') {		
		echo '';
	}
	
	
	public static function draw_textarea_field($field, $settings_group = 'hellofollowers_options', $value = '') {
		$value = esc_textarea ( stripslashes($value));
		printf('<textarea id="hellofollowers_options_%1$s" name="%2$s[%1$s]" class="input-element stretched" rows="5">%3$s</textarea>', $field, $settings_group, $value);
	}

	public static function draw_wpeditor_field($field, $settings_group = 'hellofollowers_options', $value = '') {
		$value =  stripslashes($value);

		printf('<textarea id="hellofollowers_options_%1$s" name="%2$s[%1$s]" class="input-element stretched hf-hidden-element" rows="5">%3$s</textarea>', $field, $settings_group, $value);
		$editor_options = array('textarea_name' => $settings_group.'['.$field.']');
		wp_editor($value, sprintf('wpeditor_%1$s', $field), $editor_options);
	}
	
	public static function draw_editor_field($field, $settings_group = 'hellofollowers_options', $value = '', $mode = 'htmlmixed') {
		$value = esc_textarea ( stripslashes($value));
		printf('<textarea id="hellofollowers_options_%1$s" name="%2$s[%1$s]" class="input-element stretched is-code-editor" rows="5" data-function-activate="activate__%1$s">%3$s</textarea>', $field, $settings_group, $value);
		printf('<script>
				function activate__%1$s() {
    var editor_%1$s = CodeMirror.fromTextArea(document.getElementById("hellofollowers_options_%1$s"), {
      lineNumbers: true,
      mode: "%2$s",
				lineWrapping: true,      
      matchBrackets: true,
	...
	public static function draw_switch_field($field, $settings_group = 'hellofollowers_options', $value = '', $on_text = '', $off_text = '') {
		if ($settings_group == '') { $settings_group = "hellofollowers_options"; }
		
		if ($on_text == "") { $on_text = esc_html__('On', 'hellofollowers'); }
		if ($off_text == "") { $off_text = esc_html__('Off', 'hellofollowers'); }
		...
		printf('<div class="hf-switch">
				...
				<input id="hellofollowers_options_%1$s" type="checkbox" name="%2$s[%1$s]" value="true" class="input-element checkbox" %3$s />
				</div>', $field, $settings_group, $is_checked, $on_text, $off_text, $on_switch, $off_switch);
	}
	
	public static function draw_input_field($field_id, $fullwidth = false, $settings_group = 'hellofollowers_options', $value = '', $icon = '', $class = '', $icon_position = '') {
		...
		echo '<input id="hellofollowers_options_' . $field_id . '" type="text" name="' . $settings_group . '[' . $field_id . ']" value="' . $value . '" class="input-element' . ($fullwidth ? ' stretched' : '') . ' '.$class.'" />';
	...
	public static function draw_fileselect_field($field_id, $settings_group = 'hellofollowers_options', $value = '', $icon = '', $class = '') {
		if ($icon != '') {
			printf('<span class="hf-input-prefix-icon"><i class="fa %1$s"></i></span>', $icon);
		}
		...
		echo '<input id="hellofollowers_options_' . $field_id . '" type="text" name="' . $settings_group . '[' . $field_id . ']" value="' . $value . '" class="input-element small-stretched '.$class.'" />';
		echo '<a href="#" class="button" id="hellofollowers_fileselect_' . $field_id . '">'.esc_html__('Select File', ESSB3_TEXT_DOMAIN).'</a>';
		?>
		...
		jQuery(document).ready(function($){
		    ...
		    $('#hellofollowers_fileselect_<?php echo $field_id; ?>').click(function(e) {
		        ...
		        custom_uploader.on('select', function() {
		            ...
		            $('#hellofollowers_options_<?php echo $field_id; ?>').val(attachment.url);
		        });
	...
	public static function draw_select_field($field, $listOfValues, $simpleList = false, $group = 'hellofollowers_options', $value = '') {
		
		echo '<select name="' . $group . '[' . $field . ']" class="input-element" id="hellofollowers_options_' . $field . '">';
	...
	public static function draw_checkbox_field($field, $group = 'hellofollowers_options', $value = '') {
		$is_checked = ($value == 'true') ? ' checked="checked"' : '';
		echo '<input id="hellofollowers_options_' . $field . '" type="checkbox" name="' . $group . '[' . $field . ']" value="true" ' . $is_checked . ' />';
	}

	public static function draw_checkbox_list_field($field, $listOfValues, $group = 'hellofollowers_options', $value = '') {
		if (!is_array($value)) { $value = array(); }
		
		foreach ($listOfValues as $key => $text) {
			$is_checked = in_array($key, $value) ? ' checked="checked"' : '';
			echo '<span class="hellofollowers_checkbox_list_item"><input id="hellofollowers_options_' . $field . '" type="checkbox" name="' . $group . '[' . $field . '][]" value="'.$key.'" ' . $is_checked . ' />'.$text.'</span>';
		}
	}
	
	public static function draw_checkbox_list_sortable_field($field, $listOfValues, $group = 'hellofollowers_options', $value = '') {
		if (!is_array($value)) {
			$value = array();
		}
		...
		foreach ($listOfValues as $key => $text) {
			$is_checked = in_array($key, $value) ? ' checked="checked"' : '';
			echo '<li><i class="fa fa-bars"></i><span class="hellofollowers_checkbox_list_item"><input id="hellofollowers_options_' . $field . '" type="checkbox" name="' . $group . '[' . $field . '][]" value="'.$key.'" ' . $is_checked . ' />'.$text.'<input type="hidden" name="' . $group . '[' . $field . '_order][]" value="'.$key.'|'.$text.'"/></span></li>';
		}
		echo '</ul>';
	}
	
	public static function draw_color_field($field, $group = 'hellofollowers_options', $value = '') {
		
		$value = stripslashes ( $value );
		
		echo '<input id="hellofollowers_options_' . $field . '" type="text" name="' . $group . '[' . $field . ']" value="' . $value . '" class="input-element stretched" data-default-color="' . $value . '" />';
		
		array_push ( self::$color_fields, 'hellofollowers_options_' . $field );
	}
