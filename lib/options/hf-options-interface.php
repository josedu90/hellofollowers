...
class HelloFollowersOptionsInterface {
	...
	public static function draw_form_start($custom = false, $group = '') {
		...
		$admin_template = hellofollowers_options_value('admin_template');
		if (!empty($admin_template)) {
			$admin_template = "hf-template-".$admin_template;
		}
		
		echo '<div id="hf-scroll-top"></div>';
		echo '<form id="hellofollowers_options_form" enctype="multipart/form-data" method="post" action="">';
		if ($custom && !empty($group)) {
			settings_fields( $group );
		}
		else {
			settings_fields( 'hellofollowers_settings_group' );
		}
		wp_nonce_field( 'hellofollowers_token', 'hellofollowers_token' );
		
		echo '<input id="section" name="section" type="hidden" value="'.esc_attr($active_section).'"/>';
		echo '<input id="subsection" name="subsection" type="hidden" value="'.esc_attr($active_subsection).'"/>';
		echo '<input id="tab" name="tab" type="hidden" value="'.esc_attr($current_tab).'"/>';
		echo '<div class="hf-options '.esc_attr($admin_template).'" id="hf-options">';
	}
	...
	public static function draw_header($title = '', $hide_update_button = false, $wizard_tab = false) {
		if ($hide_update_button) {
			...
			<a href="#" text="Back to top" class="button button-hf hf-button-backtotop">' . esc_html__( 'Back To Top', 'hellofollowers' ) . '</a>
			
			</div>';
		
		} 
		else {
			$update_button_text = esc_html__('Update Settings', 'hellofollowers');
			$next_prev_buttons = "";
			if ($wizard_tab) {
				$update_button_text = esc_html__('Save Settings', 'hellofollowers');
			...
			echo '<div class="hf-options-header" id="hf-options-header">
				<div class="hf-options-title">
			  	' . esc_attr($title) . '<span class="hf-options-subtitle"></span>
				</div>		
				<a href="#" text="Back to top" class="button button-hf hf-button-backtotop">' . esc_html__( 'Back To Top', 'hellofollowers' ) . '</a>
				'.$next_prev_buttons.'
				<input type="Submit" name="Submit" value="' . esc_attr($update_button_text) . '" class="button-primary" />				
			</div>';
		}
	}
	...
	public static function draw_content($options = array(), $custom = false, $user_settings = array()) {
		echo '<div class="hf-options-container">';
		...
		foreach($options as $section => $fields) {
			...
			if (count(HelloFollowersOptionsFramework::$heading_navigations) > 1) {
				...
				foreach (HelloFollowersOptionsFramework::$heading_navigations as $navigation_item) {
					echo '<a href="#'.esc_attr($navigation_item['id']).'" data-goto="'.esc_attr($navigation_item['id']).'" class="hf-internal-navigation-item">'.esc_html($navigation_item['title']).'</a>';
				}
				echo '</div></div>';
			}
