...
class HFSPADatabase {
	public static function log($date, $network, $value) {
		...
		$query = $wpdb->prepare("SELECT hfspa_id FROM $table_name WHERE hfspa_date=%s AND hfspa_network=%s", $date, $network);
	...
	public static function get_day_data($date) {
		...
		$table_name = $wpdb->prefix . HFOLLOW_SPA_TRACKER_TABLE;		
		$query = $wpdb->prepare("SELECT hfspa_network, hfspa_value FROM $table_name WHERE hfspa_date = %s", $date);
