...
if (count ( $user_filter_networks ) == 0) {
	$user_filter_networks = hellofollowers_followers()->active_social_networks();
} else {
	$user_filter = true;
}
		...
		<div class="hfspa-dashboard-panel-content">
			...
			<form method="POST">
				<table boder="0" cellpadding="6" cellspacing="0" width="100%"
					...
					<tr>
						<td>Period 1:</td>
						<td><input name="from_date" value="<?php echo esc_attr($original_from_date);?>"
							id="from_date" class="input-element center hfspa-datefield"/> <input
							name="to_date" value="<?php echo esc_attr($to_date);?>" id="to_date"
							class="input-element center hfspa-datefield"/></td>
					</tr>
					<tr>
						<td>Period 2:</td>
						<td><input name="from_date2" value="<?php echo esc_attr($original_from_date2);?>"
							id="from_date2" class="input-element center hfspa-datefield" /> <input
							name="to_date2" value="<?php echo esc_attr($to_date2);?>" id="to_date2"
							class="input-element center hfspa-datefield"/></td>
					</tr>
					<tr>
						...
						foreach ( hellofollowers_followers()->active_social_networks() as $social ) {
							...
							print '<input type="checkbox" name="filter_networks[]" value="' . esc_attr($social) . '" ' . ($is_checked ? 'checked="checked"' : '') . '/>' . esc_html($network_name) . '&nbsp;';
						}
						?></td>
					</tr>
					<tr>
			...
			<div id="hfspa-changes-graph"></div>
			<div class="hfspa-space20">&nbsp;</div>
			<div style="text-align: right;">
				<span class="hfspa-period-desc1">&nbsp;</span> Period 1
				<span class="hfspa-period-desc2">&nbsp;</span> Period 2
				</div>
			<div class="hfspa-space20">&nbsp;</div>
