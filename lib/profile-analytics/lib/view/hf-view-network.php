...
<div class="hfspa-dashboard-panel">
	...
	foreach ( hellofollowers_followers()->active_social_networks() as $social ) {
		...
		print '<a href="' . esc_url(admin_url ( 'admin.php?page=hellofollowers_spa&tab=detailed&network=' . $social )) . '" title="Start detailed network report for ' . $network_name . '">';
		print '<div class="hfspa-profile-card hfspa-profile-card-small-colored hfspa-profile-' . esc_attr($social) . ' hfspa-bg-' . esc_attr($social) . '">';
		print '<div class="hfspa-profile-icon"><i class="hfspa-icon-' . esc_attr($social) . '"></i></div>';
		print '<div class="hfspa-profile-text">';
		print '<h4>' . esc_html($network_name) . '</h4>';
...
<div class="hfspa-dashboard-panel">
		<div class="hfspa-dashboard-panel-title color1">
			<h4>Overview report for <?php echo esc_html($active_network_name); ?></h4>
			<h5>Last updated: <?php echo esc_html($last_date); ?></h5>
		</div>
		<div class="hfspa-dashboard-panel-content">
		...
		print '<div class="hfspa-network-profile hfspa-profile-' . esc_attr($social) . ' hfspa-bg-' . esc_attr($social) . '">';
		print '<div class="hfspa-profile-icon"><i class="hfspa-icon-' . esc_attr($social) . '"></i></div>';
		print '<div class="hfspa-profile-title"><h2>'.esc_html($active_network_name).'</h2><a target="_blank" title="Visit social profile" href="'.hellofollowers_followers()->create_follow_address($social).'" class="hfspa-profile-link">'.hellofollowers_followers()->create_follow_address($social).'</a></div>';
		...
		<div id="hfpsa-dynamic-graph"></div>
