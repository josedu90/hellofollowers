...
class HFSPAHelpers {
	...
	public static function value_diff_text($diff) {
		$text = esc_html__('No change', 'hellofollowers');
		
		if ($diff < 0) {
			$diff = abs($diff);
			$text = number_format(intval($diff)).' '.esc_html__(' Lost', 'hellofollowers');
		}
		else if ($diff > 0) {
			$text = number_format(intval($diff)).' '.esc_html__(' New', 'hellofollowers');
		}
	...
	public static function value_diff_text_only ($diff) {
		$text = "No change";
		if ($diff < 0) {
			$text = esc_html__(' Lost', 'hellofollowers');
		}
		else if ($diff > 0) {
			$text = esc_html__(' New', 'hellofollowers');
		}
